...
class Compare_ROIS(object):
    def __init__(self, path, input_sketch, roi, mask, orien_perc, MA_perc, ma_perc, area_perc, aspect_perc, loc_perc):
        self.path = path
        self.mask = mask
        ...
        if num_input_scars != 0 and num_scars != 0:
            ...
            for shape in input_contours_shape:                
                for num, shape2 in enumerate(contours_shape):
                    ...
                    comparisons.append([num, 1/5*(self.orien_perc * percentage_angle + self.MA_perc * percentage_MA + self.ma_perc * percentage_ma + self.area_perc * percentage_area + self.aspect_perc * percentage_aspect + self.loc_perc * percentage_in_x + self.loc_perc * percentage_in_y)])
                                 ...
                                 dbc.Col(
                                             dbc.Card(
                                                 [
                                                    ...
                                                    html.H6(children=['Location'], style={'textAlign': 'center', 'font-weight': 'normal'}),                                                                        
                                                    dcc.Slider(
                                                        id='my-slider5',
                                                        min=0,
                                                        max=2,
